...
if (!profile.includes("profile.privacy.showAudio") || !profile.includes("audioVisible ? <ProfileAudioSections")) throw new Error("Profile audio sections do not respect the canonical privacy projection.");
for (const marker of ["Hosted radio", "Published episodes", "Saved audio", "AudioMiniPlayer", "radioService.saveRadio", "podcastService.savePodcastEpisode", "enforced by Supabase RLS"]) if (!sections.includes(marker)) throw new Error(`Profile audio section missing: ${marker}`);
