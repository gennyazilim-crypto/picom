...
const localization = fs.readFileSync("src/services/localizationService.ts", "utf8");
...
for (const needle of ["profileService.updateCurrentProfile", "voiceService.subscribe", "Toggle microphone", "Feedback and redacted exports"]) if (!settings.includes(needle)) throw new Error(`Settings implementation is missing ${needle}`);
for (const needle of ["Light Theme", "Dark Theme", "System Theme"]) if (!localization.includes(needle)) throw new Error(`Settings localization is missing ${needle}`);
...
if (!app.includes("settingsService.updateSettings({ theme, accessibilitySettings, appearanceSettings })") || !app.includes("appearanceService.applyDocumentPreferences")) throw new Error("Appearance persistence is not wired.");
