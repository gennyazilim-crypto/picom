...
assert.equal(
  normalizeSteamOpenIdReturnTo(`https://auth.picom.gg/steam/callback/?nonce=${encodeURIComponent(nonce)}`),
  built.returnTo,
);
...
assert.ok(gateway.includes("resolveSteamCallbackNonce") && gateway.includes("steamAuthUpstream") && gateway.includes("buildDesktopAuthCallback"), "gateway forwards Steam callback to Edge before the desktop one-time exchange");
const nginxPath = "infra/nginx/auth.picom.gg.conf";
try {
  ...
  console.warn("Steam OpenID brand policy: skip nginx contract (infra/nginx/auth.picom.gg.conf not in tree)");
}
