...
const checks = [
  ...
  [app.includes("authSession?.user?.id ?? mockCurrentUserId"), "Supabase community access uses the authenticated user identity"],
  ...
  [!joinHandler.includes("voiceRoomsEnabled"), "an accessible Voice channel is not blocked by stale community-type UI settings"],
