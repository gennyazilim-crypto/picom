...
const compiled = ts.transpileModule(rankingSource, {
...
for (const marker of [
  ...
  "public.can_view_content_mention",
  "public.user_follows",
  "public.read_states",
  "public.audio_feed_read_states",
  "public.saved_messages",
  "public.saved_audio_items",
  "cursor_rank",
  "cursor_created_at",
  "cursor_feed_item_id",
  "feed_item_id desc",
  "visible_content_feed_engagement",
]) {
...
assert.ok(!service.includes("isMock"), "Feed service must not depend on mock-mode production fallback");
assert.ok(service.includes("FEED_BACKEND_UNAVAILABLE") || service.includes("FEED_LOAD_FAILED"), "Feed service must fail closed without demo data");
...
const base = {
  ...
  mentionedUserIds: ["viewer"],
  metrics: { reactions: 0, comments: 0, listeners: 0, mentionCount: 1 },
  isUnread: false,
  isFollowRelated: false,
};
const ranked = ranking.rankUnifiedFeedItems(
  [base, { ...base, feedItemId: "unread", isUnread: true }, { ...base, feedItemId: "followed", isFollowRelated: true }],
  Date.parse("2026-07-11T12:00:00.000Z"),
);
assert.equal(ranked[0].item.feedItemId, "followed");
assert.equal(ranked[1].item.feedItemId, "unread");
