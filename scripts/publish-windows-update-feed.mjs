...
const manifestVersion = manifestText.match(/^version:\s*['"]?([^\s'"]+)['"]?\s*$/mi)?.[1];
if (!manifestVersion) throw new Error(`Updater manifest ${manifestName} does not declare a version.`);
if (manifestVersion !== packageJson.version) {
  throw new Error(`Updater manifest ${manifestName} targets ${manifestVersion}, but package.json is ${packageJson.version}. Generate a matching manifest before publishing.`);
}
...
if (!artifactNames.some((name) => name.includes(packageJson.version))) {
  throw new Error(`Updater manifest ${manifestName} does not reference a ${packageJson.version} artifact.`);
}
