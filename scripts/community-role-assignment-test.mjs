...
const migration = read("supabase/migrations/20260710213000_community_role_assignment.sql");
...
for (const marker of ["actor_role.level < 80", "current_target_role.level >= actor_role.level", "next_role.level >= actor_role.level", "OWNER_ROLE_TRANSFER_REQUIRED", "ROLE_HIERARCHY_DENIED", "for update", "role_change", "redact_audit_reason"]) assert.ok(migration.includes(marker), `missing backend hierarchy boundary: ${marker}`);
assert.ok(permissions.includes("canAssignCommunityRole") && permissions.includes('targetRole.name === "Owner"') && permissions.includes("currentTargetRole.level < actorLevel"), "frontend hierarchy helper must hide forbidden assignments");
