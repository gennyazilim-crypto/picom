...
for (const locale of ["en", "tr", "de", "fr", "es", "it", "pt", "nl", "pl", "ru"]) {
  const content = await read(`src/i18n/locales/${locale}/notifications.json`);
  const parsed = JSON.parse(content);
  for (const key of ["desktop.friendRequestReceived.body", "desktop.dmReceived.hidden", "desktop.watchLive", "desktop.dismiss"]) {
    assert.equal(typeof parsed[key], "string", `${locale} is missing ${key}`);
  }
}
