...
for (const expected of ["requireSupabaseUser", '.eq("author_id", auth.user.id)', '.eq("uploader_id", auth.user.id)', '.eq("follower_id", auth.user.id)', '.eq("user_id", auth.user.id)', '"Cache-Control": "no-store']) if (!edge.includes(expected)) failures.push(`edge missing ${expected}`);
