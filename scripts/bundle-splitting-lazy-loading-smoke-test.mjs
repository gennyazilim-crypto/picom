...
assert(feedMain.includes("FeedLiveNowPreviewBand") && !feedMain.includes("StoryViewerModal"), "Feed must load Live Now Preview without StoryViewerModal");
...
for (const marker of ["Profile", "Mention Feed", "Settings", "VoiceRoomView", "admin", "no over-splitting", "runtime regression"]) assert(docs.toLowerCase().includes(marker.toLowerCase()), `Bundle split review is missing ${marker}`);
...
for (const prefix of ["ProfileView-", "MentionFeedMain-", "SettingsModal-", "VoiceRoomView-", "CommunityAdminDeferredSection-"]) assert(assets.some((file) => file.startsWith(prefix) && file.endsWith(".js")), `Production chunk is missing ${prefix}`);
