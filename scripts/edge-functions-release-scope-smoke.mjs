...
assert.deepEqual(release, ["client-config", "validate-file", "user-data-export"], "only V1-ready Edge Functions may be release-scoped");
assert.equal(new Set(release).size, release.length, "release function names must be unique");
assert.deepEqual(manifest.requiredSecretNames, ["PICOM_ALLOWED_ORIGINS"], "V1 Edge scope must not require conditional provider secrets");
...
for (const name of ["health", "livekit-token", "livekit-moderation", "livekit-webhook", "meeting-token", "meeting-livekit-webhook", "account-deletion", "account-deletion-finalize", "accept-invite", "moderation-helper", "notification-fanout", "webhook-message"]) assert.ok(excluded.has(name), `missing truthful V1 exclusion: ${name}`);
for (const name of ["livekit-token", "livekit-moderation", "livekit-webhook", "meeting-token", "meeting-livekit-webhook"]) assert.ok(!release.includes(name), `${name} cannot ship before its provider gate`);
const config = read("supabase/functions/client-config/index.ts");
for (const expected of ["enableRealtime: true", "enableDirectMessages: true", "enableFriends: true", "enableDiagnostics: true", "enableVoiceRooms: false", "enableScreenShare: false", "enableDiscovery: false", "enableBots: false", "enableWebhooks: false", "enableAutoUpdate: false"]) assert.ok(config.includes(expected), `client-config V1 flag mismatch: ${expected}`);
