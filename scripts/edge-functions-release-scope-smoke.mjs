...
assert.deepEqual(release, ["client-config", "validate-file", "user-data-export", "livekit-token", "livekit-moderation", "livekit-webhook"], "V1 release function inventory must match included scope");
assert.equal(new Set(release).size, release.length, "release function names must be unique");
assert.deepEqual(manifest.requiredSecretNames, ["PICOM_ALLOWED_ORIGINS", "LIVEKIT_URL", "LIVEKIT_API_KEY", "LIVEKIT_API_SECRET"], "V1 included media secret-name contract mismatch");
...
for (const name of ["health", "meeting-token", "account-deletion", "account-deletion-finalize", "accept-invite", "moderation-helper", "notification-fanout", "webhook-message"]) assert.ok(excluded.has(name), "missing truthful V1 exclusion: " + name);
for (const name of ["livekit-token", "livekit-moderation", "livekit-webhook"]) assert.ok(release.includes(name) && !excluded.has(name), name + " must be release-scoped after Task 668");
assert.equal(releaseItems.find((item) => item.name === "livekit-token")?.verifyJwt, true, "livekit-token must require user JWT");
assert.equal(releaseItems.find((item) => item.name === "livekit-moderation")?.verifyJwt, true, "livekit-moderation must require user JWT");
assert.equal(releaseItems.find((item) => item.name === "livekit-webhook")?.guard, "livekit-signature-body-hash-and-idempotency", "livekit-webhook signature guard missing");
...
console.log("V1 Edge Function release scope contract passed with member Voice/Screen functions included.");
