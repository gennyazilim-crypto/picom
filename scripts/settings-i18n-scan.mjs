...
// Keep this aligned with build-settings-locale.mjs. These names intentionally stay
// identical across locales because they are a product tier / program brand, not
// ordinary UI prose.
const MIRRORED_KEY_ALLOW = new Set([
...
for (const key of enKeys) {
  ...
  if (
    ...
    key !== "appearance.option.langEn" &&
    !MIRRORED_KEY_ALLOW.has(key)
  ) {
    mirrored.push(key);
  }
}
...
function allowText(text) {
  ...
  // Product tier name; intentionally not translated.
  if (trimmed === "PICOM Business") return true;
