...
const checks = [
  ...
  [main.includes("if (!safeMode.active)") && main.includes("if (safeModeActive) return") && app.includes("!safeMode.active"), "optional services paused"],
  [service.includes("resetSettings") && service.includes("clearCache") && service.includes("exportLogs"), "recovery actions"],
  [boundary.includes("Open Safe Mode") && boundary.includes("restartInSafeMode"), "error screen recovery"],
