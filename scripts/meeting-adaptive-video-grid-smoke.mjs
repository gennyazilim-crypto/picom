...
const qualityPolicy = read("src/services/meeting/meetingMediaQualityPolicy.ts");
...
assert.ok(tile.includes("cameraStream") && tile.includes("Camera off") && tile.includes("Connecting camera"), "shared participant tile live camera/avatar fallback missing");
...
for (const marker of ["setSubscribed", "setVideoQuality", "adaptiveStream: { pixelDensity: 1, pauseVideoInBackground: true }", "dynacast: true"]) assert.ok(voice.includes(marker), `LiveKit bandwidth application missing ${marker}`);
for (const marker of ["VideoQuality.HIGH", "VideoQuality.MEDIUM", "VideoQuality.LOW"]) assert.ok(qualityPolicy.includes(marker), `central media quality policy missing ${marker}`);
