...
assert.ok(service.includes("updateCommunitySettings") && service.includes('"update_secret_community_settings" : "update_community_settings"') && !service.includes('.from("communities")\n      .update'), "settings persistence must select a protected public or secret community RPC");
for (const marker of ["Choose icon", "Choose banner", "Visibility", "Default notifications", "Allow visitors to read public content", "Rules and join acceptance", "Save community settings", "manageCommunity"]) assert.ok(section.includes(marker), `missing settings UI field: ${marker}`);
