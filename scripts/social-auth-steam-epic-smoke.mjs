...
const accountButtons = read("src/account/components/AccountSocialLoginButtons.tsx");
...
const checks = [
  ...
  [steam.includes('normalizeSteamOpenIdReturnTo') && steam.includes("returnToNormalized === normalizeSteamOpenIdReturnTo(expectedReturnTo)"), "Steam validates return_to via canonical normalization"],
  [steam.includes('url.searchParams.get("openid.op_endpoint") === STEAM_OPENID_ENDPOINT'), "Steam validates the OpenID provider endpoint"],
  ...
  [steam.includes('action === "poll" || action === "exchange"') && epic.includes('action === "poll" || action === "exchange"') && steam.includes("consumeHandoff") && epic.includes("consumeHandoff"), "both functions expose a single-use exchange endpoint (with legacy poll compatibility)"],
  ...
  [service.includes("beginCustomOAuth") && service.includes("completeCustomOAuthCallback") && service.includes("action=exchange&nonce=") && service.includes("client.auth.setSession("), "custom flow opens login, consumes a one-time exchange, and sets the returned session"],
  [service.includes("createSocialAuthCallbackState") && service.includes("consumeSocialAuthCallbackState"), "client callback state is provider-bound and consumed before exchange"],
  ...
  [accountButtons.includes("authGatewayUrl") && accountButtons.includes("/${provider}/start?nonce="), "Account Center Steam/Epic login uses auth.picom.gg gateway"],
