...
for (const rpc of ["get_meeting_participant_snapshot","set_meeting_participant_hand_state","cleanup_stale_meeting_participants"]) assert.ok(service.includes(`\"${rpc}\"`) && databaseTypes.includes(`${rpc}:`), `service/type RPC contract missing ${rpc}`);
