...
const userCard = read("src/components/navigation/GlobalUserCard.tsx");
...
assert.match(sidebar, /aria-label=\{t\("sidebar\.navigation"\)\}/);
assert.match(sidebar, /aria-label=\{t\("sidebar\.mainNavigation"\)\}/);
assert.match(read("src/i18n/locales/en/navigation.json"), /"sidebar\.navigation"\s*:\s*"Picom global navigation"/);
assert.match(read("src/i18n/locales/en/navigation.json"), /"sidebar\.mainNavigation"\s*:\s*"Main navigation"/);
assert.match(sidebar, /GlobalUserCard/);
assert.match(sidebar, /globalNavigation\.css/);
assert.match(userCard, /global-user-card/);
assert.match(shell, /GlobalAppSidebar/);
assert.match(shell, /AppWorkspaceRouter/);
assert.match(shell, /globalNavigation\.css/);
assert.match(shell, /is-panel-mode/);
assert.match(shell, /isPanelActive/);
assert.match(router, /data-global-route/);
assert.match(app, /<AuthenticatedAppShell/);
assert.match(app, /isPanelActive=\{activeView === "rootPanel"\}/);
assert.match(app, /activeView === "events"/);
assert.doesNotMatch(sidebar, /Discord/i);
assert.match(css, /width:\s*220px/);
assert.match(sidebar, /compactSidebarQuery = "\(max-width: 1320px\)"/);
