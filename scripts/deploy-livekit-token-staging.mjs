...
const targetMigrationVersions = new Set([migrationVersion, rateLimitMigrationVersion, rateLimitKeywordFixMigrationVersion, authorizationAmbiguityFixMigrationVersion]);
...
try {
  ...
  if (!verificationRow?.member_gate || !verificationRow?.token_gate || !verificationRow?.rate_limit_gate || !verificationRow?.migration_recorded || !verificationRow?.rate_limit_migration_recorded || !verificationRow?.rate_limit_keyword_fix_recorded || !verificationRow?.authorization_ambiguity_fix_recorded) throw new Error("Hosted migration verification failed.");
