...
const targetMigrationVersions = new Set([migrationVersion, rateLimitMigrationVersion, rateLimitKeywordFixMigrationVersion, authorizationAmbiguityFixMigrationVersion, moderationAmbiguityFixMigrationVersion]);
...
try {
  ...
  const verification = await management(`/projects/${projectRef}/database/query`, {
    method: "POST",
    body: JSON.stringify({
      query: "select to_regprocedure('public.is_active_community_media_member(uuid,uuid)') is not null as member_gate, to_regprocedure('public.authorize_livekit_room(uuid,uuid,text)') is not null as token_gate, to_regprocedure('public.authorize_livekit_voice_moderation(uuid,uuid,uuid,text)') is not null as moderation_gate, to_regprocedure('public.consume_current_user_action_rate_limit(text)') is not null as rate_limit_gate, exists(select 1 from supabase_migrations.schema_migrations where version='20260712166000') as migration_recorded, exists(select 1 from supabase_migrations.schema_migrations where version='20260712166200') as rate_limit_migration_recorded, exists(select 1 from supabase_migrations.schema_migrations where version='20260712166300') as rate_limit_keyword_fix_recorded, exists(select 1 from supabase_migrations.schema_migrations where version='20260712166400') as authorization_ambiguity_fix_recorded, exists(select 1 from supabase_migrations.schema_migrations where version='20260712166500') as moderation_ambiguity_fix_recorded",
      read_only: true,
    }),
  });
  const verificationRow = Array.isArray(verification) ? verification[0] : null;
  if (!verificationRow?.member_gate || !verificationRow?.token_gate || !verificationRow?.moderation_gate || !verificationRow?.rate_limit_gate || !verificationRow?.migration_recorded || !verificationRow?.rate_limit_migration_recorded || !verificationRow?.rate_limit_keyword_fix_recorded || !verificationRow?.authorization_ambiguity_fix_recorded || !verificationRow?.moderation_ambiguity_fix_recorded) throw new Error("Hosted migration verification failed.");
