...
const rateLimitKeywordFixMigrationVersion = "20260712166300";
const authorizationAmbiguityFixMigrationVersion = "20260712166400";
const moderationAmbiguityFixMigrationVersion = "20260712166500";
const moderationRoleLevelHelperMigrationVersion = "20260712166510";
const prerequisiteMigrationVersions = ["20260711150600"];
const targetMigrationVersions = new Set([migrationVersion, rateLimitMigrationVersion, rateLimitKeywordFixMigrationVersion, authorizationAmbiguityFixMigrationVersion, moderationAmbiguityFixMigrationVersion, moderationRoleLevelHelperMigrationVersion]);
...
try {
  ...
  const verification = await management(`/projects/${projectRef}/database/query`, {
    method: "POST",
    body: JSON.stringify({
      query: "select to_regprocedure('public.is_active_community_media_member(uuid,uuid)') is not null as member_gate, to_regprocedure('public.authorize_livekit_room(uuid,uuid,text)') is not null as token_gate, to_regprocedure('public.authorize_livekit_voice_moderation(uuid,uuid,uuid,text)') is not null as moderation_gate, to_regprocedure('public.community_user_max_role_level(uuid,uuid)') is not null as moderation_level_gate, to_regprocedure('public.consume_current_user_action_rate_limit(text)') is not null as rate_limit_gate, exists(select 1 from supabase_migrations.schema_migrations where version='20260712166000') as migration_recorded, exists(select 1 from supabase_migrations.schema_migrations where version='20260712166200') as rate_limit_migration_recorded, exists(select 1 from supabase_migrations.schema_migrations where version='20260712166300') as rate_limit_keyword_fix_recorded, exists(select 1 from supabase_migrations.schema_migrations where version='20260712166400') as authorization_ambiguity_fix_recorded, exists(select 1 from supabase_migrations.schema_migrations where version='20260712166500') as moderation_ambiguity_fix_recorded, exists(select 1 from supabase_migrations.schema_migrations where version='20260712166510') as moderation_level_helper_recorded",
      read_only: true,
    }),
  });
