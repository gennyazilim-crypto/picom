...
const topLevelApis = ["getRuntimeInfo", "windowControl", "isWindowMaximized", "onWindowMaximizeStateChanged", "screenCapture", "showNotification", "tray", "startup", "file", "clipboard", "externalLinks", "deepLinks", "power", "updates"];
for (const api of topLevelApis) if (!preload.includes(`${api}:`)) throw new Error(`Preload API missing: ${api}`);
const nestedApis = ["getSources", "selectSource", "cancelSelection", "setStatus", "setMuted", "setCloseToTray", "showWindow", "quit", "onAction", "getState", "setEnabled", "pickImages", "saveText", "readText", "writeText", "openUrl", "onOpen", "onResume", "check", "download", "install", "onStateChange"];
...
if (channelKeys.length !== 27) throw new Error(`Expected 27 frozen IPC channels, found ${channelKeys.length}`);
...
if (handlerCount !== 22 || trustCheckCount !== handlerCount) throw new Error(`Every IPC handler must validate its sender (handlers=${handlerCount}, checks=${trustCheckCount})`);
for (const validator of ["isWindowAction", "parseNotificationPayload", "isTrayStatus", "parseSaveTextPayload", "parseClipboardWritePayload", "normalizeExternalUrl", "parseScreenCaptureListPayload", "parseScreenCaptureSelectionPayload", "parseScreenCaptureCancelPayload"]) {
