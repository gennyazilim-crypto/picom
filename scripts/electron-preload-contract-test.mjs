...
const topLevelApis = ["getRuntimeInfo", "windowControl", "isWindowMaximized", "onWindowMaximizeStateChanged", "screenCapture", "showNotification", "incomingCall", "tray", "startup", "file", "clipboard", "externalLinks", "deepLinks", "power", "updates", "activity"];
for (const api of topLevelApis) if (!preload.includes(`${api}:`)) throw new Error(`Preload API missing: ${api}`);
const nestedApis = ["getSources", "selectSource", "cancelSelection", "setStatus", "setMuted", "setCloseToTray", "showWindow", "quit", "onAction", "getState", "setEnabled", "pickImages", "saveText", "readText", "writeText", "openUrl", "onOpen", "onResume", "check", "download", "install", "onStateChange", "show", "dismiss", "respond", "getSnapshot"];
...
if (channelKeys.length !== 32) throw new Error(`Expected 32 frozen IPC channels, found ${channelKeys.length}`);
...
const trustCheckCount = (main.match(/isTrustedIpcEvent\(event\)/g) ?? []).length + (main.match(/isIncomingCallToastSender\(event\)/g) ?? []).length;
if (handlerCount !== 26 || trustCheckCount < handlerCount) throw new Error(`Every IPC handler must validate its sender (handlers=${handlerCount}, checks=${trustCheckCount})`);
