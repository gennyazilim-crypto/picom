...
const files = {
  ...
  directRealtimeHook: readFileSync(resolve(root, "src/hooks/useDirectMessageRealtime.ts"), "utf8"),
...
const checks = [
  ...
  [files.realtimeService.includes("createRealtimeEventDeduper"), "generic bounded event deduper"],
  [files.realtimeService.includes("directMessages") && files.realtimeService.includes("directReactions"), "central DM channel names"],
  ...
  [files.directRealtimeHook.includes("eventDeduperRef"), "DM event dedupe ref"],
  [files.directRealtimeHook.includes("Duplicate DM realtime event ignored"), "DM duplicate diagnostics"],
  [files.directRealtimeHook.includes("DM realtime subscriptions removed"), "DM subscription cleanup diagnostics"],
