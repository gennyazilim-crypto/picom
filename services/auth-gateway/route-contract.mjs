...
export function isAllowedAuthGatewayRedirect(target) {
  ...
  try {
    ...
    if (url.protocol === "picom:") {
      return url.hostname === "auth" && url.pathname === "/callback" && !url.username && !url.password && !url.hash;
    }
...
const STEAM_HANDOFF_NONCE_RE = /^[A-Za-z0-9_-]{32,128}$/;

/** @param {string} nonce */
export function isValidSteamHandoffNonce(nonce) {
  return STEAM_HANDOFF_NONCE_RE.test(String(nonce ?? "").trim());
}

/**
 * Canonical auth.picom.gg/steam/callback?nonce=… form for safe equality checks.
 * @param {string} value
 */
export function normalizeSteamOpenIdReturnTo(value) {
  const raw = String(value ?? "").trim();
  if (!raw) return null;
  try {
    const parsed = new URL(raw);
    if (parsed.protocol !== "https:" || parsed.hostname.toLowerCase() !== AUTH_GATEWAY_HOST) return null;
    const path = parsed.pathname.replace(/\/+$/, "") || "/";
    if (path !== "/steam/callback") return null;
    const nonce = parsed.searchParams.get("nonce") ?? "";
    if (!isValidSteamHandoffNonce(nonce)) return null;
    return expectedSteamReturnTo(nonce);
  } catch {
    return null;
  }
}
...
/**
 * Nonce from callback query or from signed openid.return_to when proxies reorder params.
 * @param {URLSearchParams} searchParams
 */
export function resolveSteamCallbackNonce(searchParams) {
  const direct = searchParams.get("nonce") ?? "";
  if (isValidSteamHandoffNonce(direct)) return direct.trim();
  const fromReturn = normalizeSteamOpenIdReturnTo(searchParams.get("openid.return_to") ?? "");
  if (!fromReturn) return "";
  try {
    return new URL(fromReturn).searchParams.get("nonce") ?? "";
  } catch {
    return "";
  }
}
