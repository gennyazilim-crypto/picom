...
select ok((select relrowsecurity from pg_class where oid='public.friend_requests'::regclass) and (select relrowsecurity from pg_class where oid='public.friendships'::regclass) and (select relrowsecurity from pg_class where oid='public.user_blocks'::regclass),'Friend lifecycle and blocks remain RLS protected');
