...
select is((select published_broadcasts from public.public_platform_stats_snapshots where singleton), 3::bigint, 'counts only published canonical sessions and excludes starting, aborted, terminated, excluded, and bot-host sessions');
...
select is((select count(*) from public.get_public_platform_stats() as stats where stats.registered_users = 2 and stats.active_communities = 1 and stats.published_broadcasts = 3), 1::bigint, 'anon receives validated aggregate values only');
reset role;
