...
insert into public.profiles (
  ...
  onboarding_completed, onboarding_completed_at, is_deleted, is_bot, deletion_requested_at
) values
  ('f1000000-0000-4000-8000-000000000001', 'stats-real', 'Stats Real', 'online', 'Fixture', '#007571', true, now(), false, false, null),
  ('f1000000-0000-4000-8000-000000000005', 'stats-profile-deleted', 'Stats Profile Deleted', 'offline', 'Fixture', '#007571', true, now(), true, false, null),
  ('f1000000-0000-4000-8000-000000000006', 'stats-bot', 'Stats Bot', 'offline', 'Fixture', '#007571', true, now(), false, true, null),
  ('f1000000-0000-4000-8000-000000000008', 'stats-excluded', 'Stats Excluded', 'offline', 'Fixture', '#007571', true, now(), false, false, null)
...
insert into public.platform_stats_exclusions (entity_type, entity_id, reason, source, created_at, expires_at) values
  ('user', 'f1000000-0000-4000-8000-000000000008', 'seed', 'fixture source', now(), null),
  ('user', 'f1000000-0000-4000-8000-000000000001', 'test', 'expired fixture', now() - interval '2 minutes', now() - interval '1 minute'),
  ('community', 'f2000000-0000-4000-8000-000000000003', 'seed', 'fixture source', now(), null),
  ('community', 'f2000000-0000-4000-8000-000000000004', 'moderation', 'fixture source', now(), null),
  ('session', 'f4000000-0000-4000-8000-000000000006', 'seed', 'fixture source', now(), null)
...
select ok(position('auth.users' in pg_get_functiondef('public.get_public_platform_stats()'::regprocedure)) = 0, 'public RPC does not read raw auth rows');
select ok(position('profiles' in pg_get_functiondef('public.get_public_platform_stats()'::regprocedure)) = 0, 'public RPC does not read profile rows');
...
select ok(position('count(distinct session.id)' in lower(pg_get_functiondef('public.refresh_public_platform_stats_internal()'::regprocedure))) > 0, 'broadcast aggregate uses the canonical distinct session identity');
select ok(position('pg_advisory_xact_lock' in pg_get_functiondef('public.refresh_public_platform_stats_internal()'::regprocedure)) > 0, 'refresh serializes concurrent writers with an advisory transaction lock');
...
set generated_at = now() - interval '6 minutes',
