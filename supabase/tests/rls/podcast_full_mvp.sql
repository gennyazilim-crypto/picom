...
select ok(exists(select 1 from pg_policies where schemaname='public' and tablename='podcast_playback_progress' and cmd='SELECT' and qual like '%user_id = auth.uid()%'), 'playback progress belongs only to current user');
