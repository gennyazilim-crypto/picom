...
select ok(exists(select 1 from pg_policies where schemaname='public' and tablename='friend_presence' and policyname='friend_presence_self_or_friend_read' and qual like '%share_presence%'),'presence is visible only to self or accepted friends when shared');
