...
select lives_ok(format($sql$select * from public.send_text_message_idempotent(%L,%L,'private','task510-private')$sql$,(select id from public.communities where creation_request_id='51000000-0000-4000-8000-000000000010'),'51000000-0000-4000-8000-000000000040'),'active community member can send to every community channel');
insert into public.attachments(id,uploader_id,storage_path,file_name,mime_type,size_bytes,attachment_type,status,scan_status) select '51000000-0000-4000-8000-000000000030','51000000-0000-4000-8000-000000000002',format('communities/%s/channels/%s/pending/51000000-0000-4000-8000-000000000002/image.png',community.id,channel.id),'image.png','image/png',12,'image','pending','clean' from public.communities community join public.channels channel on channel.community_id=community.id and channel.name='general' where community.creation_request_id='51000000-0000-4000-8000-000000000010';
