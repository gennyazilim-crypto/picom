...
select is((select relreplident from pg_class where oid='public.user_follows'::regclass),'f'::char,'follow deletes expose a complete reconciliation identity');
select is((select relreplident from pg_class where oid='public.saved_audio_items'::regclass),'f'::char,'saved audio deletes expose a complete reconciliation identity');
select is((select relreplident from pg_class where oid='public.audio_feed_read_states'::regclass),'f'::char,'audio read-state deletes expose a complete reconciliation identity');
