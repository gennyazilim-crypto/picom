...
-- broadcast. Deployments without this optional event metadata keep those old
-- rows excluded rather than inferring a publication from their lifecycle.
do $$
begin
  if to_regclass('public.community_events') is not null
    and exists (
      select 1
