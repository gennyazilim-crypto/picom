...
create policy "text attachment settings insert guard" on public.message_attachments as restrictive for insert to authenticated with check(exists(select 1 from public.messages message where message.id=message_id and public.text_message_setting_allows(message.channel_id,null,'attachment')));
...
insert into storage.buckets(id,name,public,file_size_limit,allowed_mime_types) values('community-branding','community-branding',true,6291456,array['image/png','image/jpeg','image/webp']) on conflict(id) do update set public=true,file_size_limit=excluded.file_size_limit,allowed_mime_types=excluded.allowed_mime_types;
