...
begin
  ...
  order by
    case when public.picom_verified_subscription_is_entitling(status, grace_until) then 0 else 1 end,
    provider_state_version desc,
    updated_at desc
  limit 1;
  ...
  if found then
    ...
    if entitling then
      if subscription_row.status in ('past_due', 'grace_period') then
        ...
        grace_until := subscription_row.grace_until;
      else
      ...
      ends_at := coalesce(subscription_row.ended_at, subscription_row.current_period_end, now());
    end if;
  end if;
  ...
  set status = case when entitling then 'expired' else entitlement_status end,
      ends_at = case when entitling then now() else coalesce(ends_at, now()) end,
      grace_until = null,
      updated_at = now(),
      version = version + 1,
