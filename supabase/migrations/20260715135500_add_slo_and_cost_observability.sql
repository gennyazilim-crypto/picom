...
create policy "slo defs admin read" on public.slo_definitions for select using (is_admin());
...
create policy "cost metrics admin read" on public.cost_metrics for select using (is_admin());
