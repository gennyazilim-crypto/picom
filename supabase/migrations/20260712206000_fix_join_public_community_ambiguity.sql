...
begin
  ...
  if not found then
    raise exception 'COMMUNITY_NOT_FOUND' using errcode = '22023';
  end if;
  if target_community.visibility <> 'public' then
    raise exception 'PRIVATE_COMMUNITY_INVITE_REQUIRED' using errcode = '42501';
  end if;
  if exists (
    select 1
    from public.community_bans ban
  ...
  then
    raise exception 'RULES_ACCEPTANCE_REQUIRED' using errcode = '42501';
  end if;
  ...
  where role_row.community_id = target_community_id
    and (role_row.is_default = true or lower(role_row.name) = 'member')
  order by role_row.is_default desc, role_row.level asc
  limit 1;

  if default_role_id is null then
    raise exception 'DEFAULT_ROLE_MISSING' using errcode = '23514';
  end if;
  ...
  values (
    ...
    public.redact_audit_reason('Public community joined after required rule gate')
  );
