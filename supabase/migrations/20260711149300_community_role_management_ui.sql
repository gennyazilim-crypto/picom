...
declare owner_id uuid; actor_role public.roles%rowtype; current_role public.roles%rowtype; actor_position integer;
begin
  ...
  select community.owner_id into owner_id from public.communities community where community.id=target_community_id;
  select role.* into actor_role from public.community_members member join public.roles role on role.id=member.role_id where member.community_id=target_community_id and member.user_id=auth.uid();
  select * into current_role from public.roles where id=target_role_id and community_id=target_community_id for update;
  actor_position:=case when owner_id=auth.uid() then 101 else coalesce(actor_role.level,0) end;
  if current_role.id is null then raise exception 'ROLE_UPDATE_INVALID' using errcode='22023'; end if;
  if current_role.system_key='owner' then raise exception 'SYSTEM_ROLE_MUTATION_FORBIDDEN' using errcode='42501'; end if;
  if owner_id<>auth.uid() and (not public.has_community_permission(target_community_id,'manageRoles') or current_role.level>=actor_position) then raise exception 'ROLE_HIERARCHY_DENIED' using errcode='42501'; end if;
  ...
  if current_role.system_key is not null and lower(btrim(target_name))<>lower(current_role.name) then raise exception 'SYSTEM_ROLE_MUTATION_FORBIDDEN' using errcode='42501'; end if;
  perform public.validate_role_permission_payload(target_community_id,target_permissions,owner_id=auth.uid());
  update public.roles set name=btrim(target_name),color=target_color,icon=nullif(target_icon,''),level=target_level,permissions=target_permissions,permissions_version=2,updated_at=now() where id=current_role.id returning * into current_role;
  insert into public.audit_log(community_id,actor_id,action_type,target_type,target_id,reason) values(target_community_id,auth.uid(),'role_change','role',current_role.id,public.redact_audit_reason(change_reason));
  return next current_role;
end; $$;
