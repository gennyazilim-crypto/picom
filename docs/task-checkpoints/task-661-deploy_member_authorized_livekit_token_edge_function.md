...
- Protected workflow: `Picom LiveKit Token Staging` run `29194842117` on 2026-07-12.
- Result: `success`; migration reconciliation, Function deployment, hosted matrix, summary, and artifact steps all passed.
- Artifact: `task-661-livekit-token-staging-evidence`, status `passed`, `containsSecrets=false`.
- Grants proved: Owner, Admin, Moderator, Member, and roleless Member; public and private Voice access; microphone, screen share, and screen-share audio.
- Denials proved: Visitor, non-member, and banned member; camera and data publication remained denied.
- HTTP controls proved: CORS, method, body, and JWT contract.
- Rate limit proved: ten requests per sixty seconds followed by `429 RATE_LIMITED` on request eleven.
- Cleanup covered nine synthetic identities and their ephemeral community data.
- The artifact reports 25 out-of-scope pending migrations; these are explicit schema-sync debt and are not represented as completed by Task 661.
...
Hosted run `29194842117` passed the corrected matrix and is the authoritative Task 661 hosted evidence.
