# Contact Form Email Report

**Status:** EDGE API LIVE / PUBLIC FORM BLOCKED UNTIL TURNSTILE SECRET

**Reviewed:** 2026-07-15

## Result

The deployed email-api accepts a constrained contact.submit intent, validates origin and payload, requires Turnstile for anonymous users, rate-limits submissions, stores the request, and queues both acknowledgement and internal notification. Hosted smoke confirmed that a request without the challenge fails closed. Public use remains blocked until TURNSTILE_SECRET_KEY is installed.
