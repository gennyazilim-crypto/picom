...
Workers atomically claim batches, record attempts, retry transient failures with backoff, suppress permanent failures, update heartbeats, and expose a local health endpoint. Multiple workers may use unique EMAIL_WORKER_ID values.
