...
Create a new mailbox password in Spacemail, update Supabase Auth SMTP and /etc/picom/email-worker.env through protected operator channels, restart worker, verify TLS/auth/test delivery, then revoke the old password. Rotate unsubscribe/rate salts separately with an overlap plan for existing links.
