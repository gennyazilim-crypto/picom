...
2. compares hosted migration history with the ordered repository migration set, requires Voice prerequisite `20260711150600` plus the `community_voice_rooms_enabled(uuid)` and `can_read_public_channel(uuid)` RPCs, and applies only reviewed active-member migration `20260712166000`, rate-limit reconciliation `20260712166200`, reserved-keyword correction `20260712166300`, and authorization-column ambiguity correction `20260712166400` through the official Supabase Management API database query endpoint. Other pending versions are reported as `outOfScopePendingMigrations` instead of being executed or presented as complete; an existing schema without migration history still fails closed;
3. verifies migrations `20260712166000` through `20260712166400` plus the member, token, and rate-limit RPCs;
...
This Task 661 workflow is not a replacement for a full owner-authorized `supabase db push`. Its only completion requirement is that the reviewed Voice prerequisite and RPCs exist, active-member migration `20260712166000` is recorded, the content-free 10/60 token limiter is reconciled by `20260712166200` plus `20260712166300`, and the `RETURNS TABLE` output-column collision is removed by `20260712166400` before token deployment. `outOfScopePendingMigrations` and deferred Storage-owner migrations remain explicit release debt and must be completed through the canonical database-owner path before a final release gate can claim full schema synchronization.
