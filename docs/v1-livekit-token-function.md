...
2. compares hosted migration history with the ordered repository migration set, applies and records only pending migrations through the official Supabase Management API database query endpoint, and fails closed if an existing schema has no migration history;
