...
const fail = <T>(code: string, message: string, retryAfterMs?: number): MeetingControlResult<T> => ({ ok: false, error: { code, message, ...(retryAfterMs ? { retryAfterMs } : {}) } });
...
export const meetingControlService = {
  ...
  async cancelShareSelection(requestId:string):Promise<void>{if(!dataSourceService.getStatus().isMock)await screenCaptureService.cancelSelection(requestId);},
  ...
  async sendReaction(kind:MeetingReactionKind):Promise<MeetingControlResult>{const result=await meetingService.sendReaction(kind);return result.ok?ok(true):fail(result.error.code,result.error.message,result.error.retryAfterMs);},
  async toggleHand():Promise<MeetingControlResult>{const participant=localParticipant();if(!participant)return fail("MEETING_PARTICIPANT_MISSING","Your meeting participant is not connected.");const snapshot=meetingService.store.getSnapshot(),stageAudience=snapshot.context?.roomMode==="stage"&&!(["host","cohost","speaker"] as const).includes(participant.role as "host"|"cohost"|"speaker"),entry=snapshot.stageQueue?.find((item)=>item.participantId===participant.id),action=stageAudience?(entry?.stageRequestStatus==="requested"?"cancel_stage":"request_stage"):(participant.handRaised?"lower":"raise"),raised=action==="raise"||action==="request_stage";if(dataSourceService.getStatus().isMock){const now=new Date().toISOString(),nextEntry={participantId:participant.id,userId:participant.userId??null,displayName:participant.displayName,meetingRole:participant.role,handRaised:raised,handRaisedAt:raised?(entry?.handRaisedAt??now):null,handSequence:raised?(entry?.handSequence||((snapshot.stageQueue?.length??0)+1)):0,acknowledgedByUserId:null,acknowledgedAt:null,stageRequestStatus:action==="request_stage"?"requested":action==="cancel_stage"?"cancelled":entry?.stageRequestStatus??"none",stageRequestedAt:action==="request_stage"?now:entry?.stageRequestedAt??null,stageResolvedAt:action==="cancel_stage"?now:entry?.stageResolvedAt??null,stageResolvedByUserId:null,serverVersion:(entry?.serverVersion??0)+1,updatedAt:now} as const;meetingService.store.replaceParticipants(snapshot.generation,snapshot.participantIds.map((id)=>snapshot.participantsById[id]).filter(Boolean).map((item)=>item.id===participant.id?{...item,handRaised:raised}:item));meetingService.store.patch(snapshot.generation,{handRaised:raised,stageQueue:[...(snapshot.stageQueue??[]).filter((item)=>item.participantId!==participant.id),nextEntry]});return ok(true);}const result=await meetingService.applyHandAction(participant.id,action);return result.ok?ok(true):fail(result.error.code,result.error.message,result.error.retryAfterMs);},
  async acknowledgeHand(participantId:string):Promise<MeetingControlResult>{const snapshot=meetingService.store.getSnapshot(),entry=snapshot.stageQueue?.find((item)=>item.participantId===participantId);if(!entry?.handRaised)return fail("MEETING_HAND_NOT_RAISED","This participant no longer has a raised hand.");if(dataSourceService.getStatus().isMock){const now=new Date().toISOString();meetingService.store.patch(snapshot.generation,{stageQueue:(snapshot.stageQueue??[]).map((item)=>item.participantId===participantId?{...item,acknowledgedByUserId:"mock-host",acknowledgedAt:now,serverVersion:item.serverVersion+1,updatedAt:now}:item)});return ok(true);}const result=await meetingService.applyHandAction(participantId,"acknowledge");return result.ok?ok(true):fail(result.error.code,result.error.message,result.error.retryAfterMs);},
