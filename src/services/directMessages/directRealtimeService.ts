...
/** Supabase reuses channels by topic; callbacks cannot be added after subscribe(). */
function uniqueRealtimeChannelName(baseName: string): string {
  realtimeSubscriptionSequence += 1;
  return `${baseName}:subscription-${realtimeSubscriptionSequence}`;
}
...
export function subscribeToActiveDirectConversation(input: ActiveSubscribeInput): () => void {
  ...
  void (async () => {
    ...
    channel = client.channel(uniqueRealtimeChannelName(realtimeChannelNames.directActive(input.conversationId)))
...
export function subscribeToDirectConversationList(input: ListSubscribeInput): () => void {
  ...
  const channel = client.channel(uniqueRealtimeChannelName(realtimeChannelNames.directList(input.currentUserId)))
