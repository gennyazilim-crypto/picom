...
function statusMessage(status: ResetLinkStatus): string | null {
  if (status === "checking") return "Güvenli sıfırlama bağlantısı kontrol ediliyor…";
  if (status === "missing") return "Bu şifre sıfırlama bağlantısı eksik. Yeni bir e-posta isteyebilirsin.";
  if (status === "expired") return "Bu şifre sıfırlama bağlantısının süresi dolmuş.";
  if (status === "used") return "Bu bağlantı daha önce kullanılmış.";
  if (status === "network") return "İşlem tamamlanamadı. Lütfen tekrar dene.";
  if (status === "invalid") return "Bağlantı tamamlanamadı. Lütfen tekrar dene.";
  if (status === "success") return "PICOM şifren güncellendi. Giriş sayfasına yönlendiriliyorsun…";
  return null;
}
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (revokeOthers) {
        ...
        if (revokeError) {
          setError("Şifren güncellendi ancak diğer oturumlar kapatılamadı. Güvenlik ayarlarından oturumları kontrol et.");
          return;
        }
