...
function DirectAvatar({ conversation, large = false, compact = false }: { conversation: DirectConversation; large?: boolean; compact?: boolean }) {
  const verifiedType = getUserVerificationVariant(conversation.participantUserId)
    ?? (conversation.participantVerified ? "verified_user" : null);
  return <span className={`direct-avatar-shell status-${conversation.participantStatus} ${large ? "large" : ""} ${compact ? "compact" : ""}`}><VerifiedAvatarFrame userId={conversation.participantUserId} label={conversation.participantName} avatarUrl={conversation.participantAvatarUrl} size="medium" avatarSize={large ? 64 : compact ? 38 : 42} verifiedType={verifiedType} /><i className="direct-status-dot" aria-hidden="true" /></span>;
}
