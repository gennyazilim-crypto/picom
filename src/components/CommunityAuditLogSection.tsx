...
export function CommunityAuditLogSection({ communityId, canView }: { communityId: string; canView: boolean }) {
  ...
  useEffect(() => {
    let active = true;
    void auditLogService.list(communityId, canView).then((result) => {
      if (!active) return;
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((record) => (!actor.trim() || record.actorId.toLowerCase().includes(actor.trim().toLowerCase())) && (action === "all" || record.actionType === action) && (!cutoff || Date.parse(record.createdAt) >= cutoff));
  }, [action, actor, range, records]);

  const copy = async () => {
    const exported = auditLogService.exportForAdmin(filtered, canView);
    if (!exported.ok) {
      setNotice(exported.message);
      return;
    }
  ...
  const download = () => {
    ...
    anchor.href = url;
    anchor.download = `picom-audit-${communityId.replace(/[^a-zA-Z0-9_-]/g, "-").slice(0, 80)}.json`;
    anchor.click();
    URL.revokeObjectURL(url);
    setNotice("Filtered audit JSON exported.");
  };

  const exportDisabled = !canView || filtered.length === 0;

  return (
    <section className="community-admin-section audit-log-section">
      ...
      <div className="audit-log-toolbar">
        <label><span>Actor</span><input value={actor} onChange={(event) => setActor(event.target.value)} placeholder="Filter actor ID" /></label>
