...
export function MemberModerationModal({ member, action, onClose, onConfirm }: MemberModerationModalProps) {
  ...
  return <div className="member-moderation-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <form className="member-moderation-modal" role="alertdialog" aria-modal="true" aria-labelledby="member-moderation-title" onSubmit={async (event) => {
      ...
      <label>Reason<textarea value={reason} onChange={(event) => setReason(event.target.value)} maxLength={500} rows={4} autoFocus placeholder="Add a clear moderation reason" /><small>{reason.trim().length}/500 · minimum 3 characters</small></label>
