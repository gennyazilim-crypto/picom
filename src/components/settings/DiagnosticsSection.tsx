...
export function DiagnosticsSection({ onNotice }: { onNotice: (message: string, tone?: "info" | "success" | "error") => void }) {
  ...
  return (
    <section className="diagnostics-section">
      <header>
        <div><p className="eyebrow">Support snapshot</p><h3>Diagnostics</h3></div>
        <button type="button" onClick={() => setSnapshot(diagnosticsService.getSnapshot())}>Refresh</button>
      </header>
      <div className="diagnostics-grid">
        ...
        <article><span>Window</span><strong>{snapshot.runtime.window.width ?? "?"}x{snapshot.runtime.window.height ?? "?"} / {snapshot.runtime.window.focused ? "focused" : "background"}</strong></article>
      ...
      <p className="diagnostics-privacy-note"><AppIcon name="lock" size="sm" /> Secrets, passwords, tokens, cookies, private keys, and authorization headers are excluded.</p>
