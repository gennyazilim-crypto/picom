...
export function InvitePeopleModal({ community, currentUserId, canCreate, onClose }: { community: Community; currentUserId: string; canCreate: boolean; onClose: () => void }) {
  ...
  return <InviteModalShell title={`Invite people to ${community.name}`} onClose={onClose}><div className="invite-flow-content"><p>Create a limited invite. Private community content remains protected until the invite is accepted.</p><div className="invite-options"><label><span>Maximum uses</span><input type="number" min="1" max="100" value={maxUses} onChange={(event) => setMaxUses(event.target.value)} /></label><label><span>Expires in days</span><input type="number" min="1" max="30" value={expiresInDays} onChange={(event) => setExpiresInDays(event.target.value)} /></label></div>{invite ? <div className="invite-result"><span>Invite link</span><strong>{communityInviteService.getInviteLink(invite.code)}</strong><button type="button" onClick={() => void copy()}><AppIcon name="paperclip" size="sm" /> Copy link</button></div> : null}{message ? <p className="invite-flow-message" role="status">{message}</p> : null}<div className="modal-actions-row"><button className="secondary-action" type="button" onClick={onClose}>Close</button><button className="send-button" type="button" disabled={!canCreate || busy} onClick={() => void create()}><AppIcon name="plus" size="sm" />{busy ? "Creating…" : "Create invite"}</button></div></div></InviteModalShell>;
}
