...
export function OnboardingFlow({
  ...
  const canContinue = (currentStep.id !== "profile" || profile.displayName.trim().length > 0)
    && (currentStep.id !== "community" || startChoice !== "joinInvite" || inviteCode.trim().length > 0);
  ...
  const stepLabels: Record<string, string> = {
    ...
    theme: t("onboarding.stepTheme"),
    community: t("onboarding.stepCommunity"),
    follow: t("onboarding.stepFollow"),
    ...
    <main className="first-run-onboarding" aria-label="Picom first-run onboarding">
      <section className="onboarding-wizard" aria-live="polite">
        <aside className="onboarding-progress-panel">
          ...
          <div>
            <p className="eyebrow">{t("onboarding.desktop")}</p>
            <h1>{t("onboarding.makeYours")}</h1>
            <p>{t("onboarding.summary")}</p>
          </div>
          <ol>
            {steps.map((step, index) => (
              ...
              >
                ...
                <strong>{stepLabels[step.id] ?? step.label}</strong>
              </li>
            ))}
          </ol>
          ...
          <footer className="onboarding-footer">
            ...
            {currentStep.id === "finish" ? (
              ...
              <button
                ...
                onClick={advance}
              >
                {t("action.next")} <AppIcon name="chevronRight" size="sm" />
              </button>
            )}
          </footer>
        </div>
