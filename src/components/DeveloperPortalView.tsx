  ...
  const content = active === "bots"
    ...
    : active === "webhooks"
      ...
      : active === "applications"
        ...
        : <div className="placeholder-panel action-panel"><strong>API Docs placeholder</strong><p>Architecture documentation covers role-scoped bots and incoming webhooks. No public API base URL or production developer credential is exposed.</p><div className="settings-status-card"><span>Bot API</span><strong>Foundation only</strong><small>One-time credentials, role permissions, server verification, and rate limits must be production-certified.</small></div><div className="settings-status-card"><span>Webhook API</span><strong>Server-gated foundation</strong><small>Use one-time URLs securely; delivery requires explicit protected backend enablement.</small></div></div>;

  return <div className="modal-backdrop" onMouseDown={onClose}><section className="settings-modal" role="dialog" aria-modal="true" aria-labelledby="developer-portal-title" onMouseDown={(event) => event.stopPropagation()}><aside className="settings-nav"><p className="eyebrow">Future developer tools</p><h2 id="developer-portal-title">Developer Portal</h2><div className="settings-tabs">{sections.map((section) => <button type="button" key={section.id} className={active === section.id ? "active" : ""} onClick={() => setActive(section.id)}><AppIcon name={section.icon} size="sm" />{section.label}</button>)}</div></aside><main className="settings-content"><button type="button" className="icon-button modal-close" aria-label="Close Developer Portal" onClick={onClose}><AppIcon name="close" size="lg" /></button><p className="eyebrow">Development / restricted</p><h2>{sections.find((section) => section.id === active)?.label}</h2>{content}</main></section></div>;
}
