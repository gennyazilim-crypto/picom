...
export function EmojiPicker({ label, className, onSelect, onClose, communityId, mode = "composer" }: EmojiPickerProps) {
  ...
  useEffect(() => {
    ...
    setFocusedIndex(0);
  }, [activeCategory, query, visibleOptions.length]);
  ...
  return (
    <div ref={pickerRef} className={`emoji-picker ${className ?? ""}`} role="dialog" aria-label={label} data-mode={mode}>
      <div className="emoji-picker-header">
        <div>
          <strong>{mode === "reaction" ? "Add reaction" : "Insert emoji"}</strong>
          <span>Search or use arrow keys to choose</span>
        </div>
        <span className="emoji-picker-mode">{mode === "reaction" ? "Reaction" : "Message"}</span>
      </div>
      ...
      <div className="emoji-picker-grid" role="listbox" aria-label={query ? "Search results" : emojiCategoryLabels[activeCategory]} onKeyDown={onGridKeyDown}>
