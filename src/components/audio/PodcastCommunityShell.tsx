...
type PodcastSection = "episodes" | "series" | "drafts" | "hosts" | "about" | "listenerDiscussion";
type PodcastCommunityShellProps = { community: Community; canPublish: boolean; onOpenProfile?: (member: Member) => void };

const sections: readonly { id: Exclude<PodcastSection, "drafts" | "listenerDiscussion">; label: string; icon: IconName }[] = [
...
export function PodcastCommunityShell({ community, canPublish, onOpenProfile }: PodcastCommunityShellProps) {
  const [activeSection, setActiveSection] = useState<PodcastSection>("episodes");
  ...
  if (selectedEpisode && snapshot) {
    ...
    return <PodcastEpisodeDetail episode={selectedEpisode} communityName={community.name} author={author} relatedEpisodes={[...snapshot.episodes]} getCommentAuthorLabel={getUserLabel} onClose={() => setSelectedEpisode(null)} onOpenCommunity={() => setSelectedEpisode(null)} onOpenAuthor={author && onOpenProfile ? (_event, member) => onOpenProfile(member) : undefined} onSelectEpisode={setSelectedEpisode} />;
  }
  ...
  return <main className="podcast-community-shell">
    ...
    <div className="podcast-shell-layout">
      <nav className="podcast-shell-navigation" aria-label={`${community.name} podcast sections`}>
        <span>LIBRARY</span>
        {navigation.map((section) => <button key={section.id} type="button" className={activeSection === section.id ? "active" : ""} aria-current={activeSection === section.id ? "page" : undefined} onClick={() => setActiveSection(section.id)}><AppIcon name={section.icon} size="md" /><span>{section.label}</span></button>)}
