...
export function CommunitySettingsEditor({ community, access, onUpdated }: Props) {
  ...
  return <div className="community-settings-form community-settings-editor">
    ...
    <div className="community-settings-grid"><label><span>Visibility</span><select value={visibility} disabled><option value="public">Public</option><option value="private">Private</option><option value="secret">Secret (managed secure lifecycle)</option></select></label><label><span>Default notifications</span><select value={defaultNotificationLevel} onChange={(event) => setDefaultNotificationLevel(event.target.value as CommunityNotificationLevel)}><option value="all">All activity</option><option value="mentions">Mentions</option><option value="none">None</option></select></label></div>
    <label className="moderation-filter-toggle"><input type="checkbox" checked={visibility === "public" && publicReadEnabled} disabled={visibility !== "public"} onChange={(event) => setPublicReadEnabled(event.target.checked)} /><span>Allow visitors to read public content</span></label>
