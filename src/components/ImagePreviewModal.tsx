  ...
  return (
    <div className="image-preview-backdrop" onMouseDown={onClose}>
      <figure
        ...
        role="dialog"
        aria-modal="true"
        aria-label={localizationService.translate("feed.media.viewerLabel")}
        tabIndex={-1}
        onMouseDown={(event) => event.stopPropagation()}
      >
        <button type="button" className="icon-button modal-close" aria-label={localizationService.translate("feed.media.close")} onClick={onClose}>
          <AppIcon name={overlayIcons.close} size="lg" />
        </button>
        {hasNav ? (
          <>
            <button type="button" className="image-preview-nav previous" aria-label={localizationService.translate("feed.media.previous")} onClick={() => go(-1)}>
              <AppIcon name="chevronRight" size="md" />
            </button>
            <button type="button" className="image-preview-nav next" aria-label={localizationService.translate("feed.media.next")} onClick={() => go(1)}>
              <AppIcon name="chevronRight" size="md" />
            </button>
          </>
        ) : null}
        {access.canRender && !broken ? (
          ...
          <div className="attachment-card-blocked" role="status">
            <strong>{broken ? localizationService.translate("feed.media.broken") : access.title}</strong>
            <span>{broken ? localizationService.translate("feed.media.brokenBody") : access.message}</span>
          </div>
        )}
        <figcaption>
          ...
          {access.canRender && !broken ? (
            ...
            >
              ...
              {downloading ? localizationService.translate("feed.media.downloading") : localizationService.translate("feed.media.download")}
            </button>
          ) : null}
        </figcaption>
      </figure>
      {menu && access.canRender && !broken ? (
        <DesktopContextMenu
          ...
          items={[
            {
              label: downloading ? localizationService.translate("feed.media.downloading") : localizationService.translate("feed.media.download"),
              icon: "image",
              disabled: downloading,
