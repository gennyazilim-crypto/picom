  ...
  return (
    ...
    >
      <div className="live-watch-chat__header">
        ...
        <button type="button" className="live-watch-controls__btn" aria-label={open ? "Close chat" : "Open chat"} aria-expanded={open} onClick={onToggle}>
        ...
        <>
            ...
            {messages.length === 0 ? (
              ...
              messages.map((message) => (
                <article key={message.id} className={`live-watch-chat__item ${message.authorId === currentUserId ? "is-own" : ""}`}>
                  <span className="live-watch-chat__author">{authorLabel(message, currentUserId, profiles)}</span>
          ...
          >
            ...
            <input
              ...
              placeholder={readOnly ? "Chat is read-only" : connection === "denied" ? "Chat unavailable" : "Send a message"}
              disabled={readOnly || connection === "denied" || sending}
              maxLength={2000}
            />
            <button type="submit" className="live-watch-btn live-watch-btn--primary" disabled={readOnly || connection === "denied" || sending || !draft.trim()} aria-label="Send message">
