...
export function LiveWatchEndedState({ session, onBackToLiveNow, onOpenCommunity }: LiveWatchEndedStateProps) {
  const name = session ? broadcasterLabel(session) : "Broadcaster";
  return (
    <div className="live-watch-ended" role="status" aria-live="polite">
      <div className="live-watch-ended__card">
        ...
        <strong>Yayın sona erdi</strong>
        <p>{session?.title.trim() ? `“${session.title.trim()}” has ended.` : "This live stream has ended."}</p>
...
export function LiveWatchHeader({
  ...
  return (
    <header className="live-watch-header">
      <button type="button" className="live-watch-btn" onClick={onBack} aria-label="Back to Live Now">
...
export function LiveWatchReportMenu({
  ...
  if (!open) return null;
  return (
    <div className="live-watch-report" role="dialog" aria-modal="true" aria-label="Report live stream">
      <div className="live-watch-report__panel">
        <div className="live-watch-report__head">
          <strong>Report stream</strong>
          <button type="button" className="live-watch-controls__btn" aria-label="Close report menu" onClick={onClose}>
