...
import { AdminInfrastructureHealth, AdminOperationsPagedList, AdminSystemStatusV2 } from "./AdminOperationsV2Sections";
...
const sections: Array<{ id: SectionId; label: string; icon: IconName }> = [
  ...
  { id: "verification", label: "Verification Requests", icon: "lock" },
...
export function AdminOperationsPanel({ access }: { access: AdminOperationsAccess }) {
  ...
  const content = active === "productHealth"
    ...
    : active === "system"
    ? <><div className="admin-ops-metrics"><article><span>Access source</span><strong>{access.source}</strong></article><article><span>Data source</span><strong>{snapshot.dataSource}</strong></article><article><span>Network</span><strong>{snapshot.network.state}</strong></article><article><span>Supabase</span><strong>{snapshot.serviceHealth.supabase}</strong></article><article><span>LiveKit</span><strong>{snapshot.serviceHealth.liveKit}</strong></article><article><span>Release</span><strong>{snapshot.serviceHealth.version} - {snapshot.serviceHealth.releaseChannel}</strong></article></div><div className="admin-ops-detail"><strong>Safe service summary</strong><p>Supabase host: {snapshot.serviceHealth.supabaseHost ?? "Not configured"}. No credentials, tokens, private configuration, or message content are loaded.</p></div><AdminInfrastructureHealth access={access} /><AdminSystemStatusV2 access={access} /></>
