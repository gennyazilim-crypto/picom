...
export function MeetingRightDock({snapshot,onSelect,onFocusParticipant,onClose}:{snapshot:MeetingClientSnapshot;onSelect:(panel:MeetingSidePanel)=>void;onFocusParticipant:(id:string|null)=>void;onClose:()=>void}){
  ...
  return <aside id="meeting-side-panel" tabIndex={-1} className="meeting-right-dock meeting-right-dock-v2" aria-label={`Meeting ${labels[active]} panel`}><header><div><small>Meeting dock</small><strong>{labels[active]}</strong></div><button type="button" aria-label="Close meeting side panel" onClick={onClose}><AppIcon name="close" size="sm"/></button></header><nav aria-label="Meeting panel sections">{tabs.map((tab)=><button type="button" key={tab} className={active===tab?"active":""} aria-current={active===tab?"page":undefined} aria-keyshortcuts={tab==="people"?"P":tab==="chat"?"C":undefined} onClick={()=>select(tab)}>{labels[tab]}{tab==="people"?<span aria-label={`${snapshot.participantIds.length} connected`}>{snapshot.participantIds.length}</span>:null}</button>)}</nav><div className="meeting-right-dock__content">{active==="people"?<MeetingPeoplePanel snapshot={snapshot} onFocusParticipant={onFocusParticipant}/>:active==="chat"?<MeetingChatPanel snapshot={snapshot}/>:active==="captions"?<MeetingCaptionPanel caption={caption}/>:active==="info"?<MeetingInfoPanel snapshot={snapshot}/>:null}</div></aside>;
}
