...
import { MeetingMediaPrivacyIndicator } from "./MeetingMediaPrivacyIndicator";
...
export function MeetingTopBar({snapshot,captions,focusMode,onToggleFocus,onToggleDock,onMinimize}:{snapshot:MeetingClientSnapshot;captions:MeetingCaptionSnapshot;focusMode:boolean;onToggleFocus:()=>void;onToggleDock:()=>void;onMinimize?:()=>void}) {
  ...
  return <header className="meeting-top-bar">
    ...
    <div className="meeting-top-bar__meta">{snapshot.deviceNotice?<span className="meeting-device-notice" role="status" title={snapshot.deviceNotice}><AppIcon name="settings" size="xs" />{snapshot.deviceNotice}</span>:null}<MeetingMediaPrivacyIndicator snapshot={snapshot} captions={captions}/><span aria-label={`${participantCount} connected participants`}><AppIcon name="users" size="xs" />{participantCount}</span>{snapshot.role?<span aria-label={`Your meeting role: ${snapshot.role}`}>Role: {snapshot.role}</span>:null}</div>
    <div className="meeting-top-bar__actions">
      ...
      <button type="button" data-meeting-panel-toggle aria-keyshortcuts="C P" aria-label={snapshot.rightDock==="none"?"Open meeting side panel":"Close meeting side panel"} aria-pressed={snapshot.rightDock!=="none"} onClick={onToggleDock}><AppIcon name="users" size="sm" /></button>
    </div>
  </header>;
}
