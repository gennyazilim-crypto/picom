...
export function ConnectedMeetingMiniCard({ onReturn, onLeave }: { onReturn: () => void; onLeave: () => void }) {
  ...
  return <aside className={`connected-meeting-mini${terminal ? " is-terminal" : ""}`} aria-label="Connected meeting controls">
    ...
    {!terminal ? <div className="connected-meeting-mini__states"><span><AppIcon name={snapshot.localMedia.muted ? "volumeOff" : "microphone"} size="xs" />{snapshot.localMedia.muted ? "Muted" : "Mic on"}</span>{snapshot.capabilities.canPublishVideo ? <span><AppIcon name="eye" size="xs" />{snapshot.localMedia.cameraEnabled ? "Camera on" : "Camera off"}</span> : null}{snapshot.capabilities.canPublishAudio ? <span title={snapshot.noiseShield.fallbackReason??undefined}><AppIcon name="voice" size="xs" />Shield: {snapshot.noiseShield.status==="fallback"||snapshot.noiseShield.status==="fallback-standard"?"Standard fallback":noiseShieldLabels[snapshot.noiseShield.appliedMode]}</span> : null}{sharing ? <span><AppIcon name="maximize" size="xs" />Sharing</span> : null}</div> : null}
    ...
    {connected ? <div className="connected-meeting-mini__controls">
      ...
      {snapshot.capabilities.canPublishAudio ? <button type="button" disabled={busy !== null||snapshot.noiseShield.availableModes.length<2} aria-label={`Change Noise Shield mode. Applied ${noiseShieldLabels[snapshot.noiseShield.appliedMode]}`} aria-pressed={snapshot.noiseShield.appliedMode!=="off"} onClick={() => void run("noise", () => meetingControlService.setNoiseShieldMode(nextNoiseShieldMode(snapshot.noiseShield.availableModes,snapshot.noiseShield.requestedMode)))}><AppIcon name="voice" size="sm" /></button> : null}
      {snapshot.capabilities.canShareScreen || sharing ? <button type="button" aria-label={sharing ? "Return to active screen share" : "Open screen share controls"} aria-pressed={sharing} onClick={onReturn}><AppIcon name="maximize" size="sm" /></button> : null}
