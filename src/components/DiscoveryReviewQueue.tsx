...
export function DiscoveryReviewQueue({ access }: { access: AdminOperationsAccess }) {
  ...
  const applyReview = async (item: DiscoveryReviewItem) => {
    ...
    if (nextStatus === "approved" && !item.category) { setNotice("A reviewed category is required before approval."); return; }
  ...
  return <section aria-label="Restricted discovery moderation queue"><div className="admin-ops-detail"><strong><AppIcon name="search" size="sm" /> Discovery review queue</strong><p>Safe public profile metadata, content flags, and aggregate report counts only. Private communities, report descriptions, member data, messages, and attachments are excluded.</p><div className="settings-actions-row"><select value={filter ?? "all"} onChange={(event) => setFilter(event.target.value === "all" ? null : event.target.value as DiscoveryReviewStatus)}><option value="all">All states</option>{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select><button type="button" onClick={() => void refresh()}>Refresh</button></div></div>{notice ? <p className="bots-admin-notice" role="status">{notice}</p> : null}<div className="admin-ops-log-list">{loading ? <div className="admin-ops-detail"><strong>Loading queue</strong><p>Authorization is checked before safe metadata is returned.</p></div> : items.length ? items.map((item) => <article key={item.communityId}><div><strong>{item.communityName}</strong><span>{item.category} - {item.status} - {item.reportCount} reports - submitted {dateTimeService.formatFullTimestamp(item.submittedAt)}</span><small>{item.description}</small>{item.contentFlags.length ? <small>Content flags: {item.contentFlags.join(", ")}</small> : <small>Content flags: none declared</small>}</div><div className="settings-actions-row"><select aria-label={`Review status for ${item.communityName}`} value={drafts[item.communityId] ?? item.status} onChange={(event) => setDrafts((current) => ({ ...current, [item.communityId]: event.target.value as DiscoveryReviewStatus }))}>{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select><input aria-label={`Review note for ${item.communityName}`} value={reasons[item.communityId] ?? ""} maxLength={500} placeholder="Reason (recommended)" onChange={(event) => setReasons((current) => ({ ...current, [item.communityId]: event.target.value }))} /><button type="button" onClick={() => void applyReview(item)}>Apply</button></div></article>) : <div className="admin-ops-detail"><strong>No listings in this state</strong><p>Private and unlisted communities never enter this queue.</p></div>}</div></section>;
}
