...
const allHelpTopics: readonly HelpTopic[] = [
  ...
  { id: "voice-screen-share", category: "Voice", title: "Voice and screen sharing", summary: "Join member voice rooms, choose a desktop source, and recover safely when permissions or connectivity fail.", steps: ["Open a voice channel in a community where you are an active member, then select Join room. Ordinary members do not need an Owner, Admin, or Moderator role.", "Grant microphone access only after Picom asks. Review the selected input, output, microphone level, speaker test, and Standard Noise Shield under Settings > Voice & Audio.", "To share, open the room controls, choose a validated screen or window in the desktop picker, refresh the source list if needed, then select Start sharing. Picom V1 does not capture camera, system audio, or recordings.", "Use the active sharer switcher to focus a remote share. Select Stop sharing to end your local track; leaving the room also clears local media and subscriptions.", "If the room reconnects, keep Picom open while it restores the session. If recovery fails, leave and rejoin, confirm the network is online, and export redacted diagnostics.", "On Windows, open Settings > Privacy & security > Microphone and allow desktop apps. If protected windows or screens are absent from the picker, close protected content, refresh sources, restart Picom, and retry."] },
  { id: "troubleshooting", category: "Support", title: "Troubleshooting", summary: "Recover from startup, network, realtime, upload, or package problems.", steps: ["Confirm Picom version, platform, network state, and data-source mode.", "Restart normally; use Safe Mode only when local settings or optional services block startup.", "Record reproducible steps and export redacted diagnostics before contacting support."] },
