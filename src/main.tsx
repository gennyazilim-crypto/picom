...
function getRootElement(): HTMLElement {
  ...
  if (!rootElement) throw new Error("Picom renderer root element was not found.");
  return rootElement;
}
...
function scheduleOptionalRendererServices(safeModeActive: boolean): void {
  const start = () => {
    ...
    void import("./services/sleepWakeResumeService").then((module) => module.sleepWakeResumeService.start()).catch(() => {
...
function bootstrapRenderer(): void {
  markRuntime();
  socialAuthService.startNativeOAuthListener();
  ...
  if (!safeMode.active) deepLinkService.startNativeListener();
  ReactDOM.createRoot(getRootElement()).render(
    <React.StrictMode>
      <DesktopStartupErrorBoundary>
        <App />
