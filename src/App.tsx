...
import { accountPrivacySetupService } from "./services/privacy/accountPrivacySetupService";
...
} from "./services/firstLaunchHandoffIntent";
import {
  defaultFirstProductActionCapabilities,
  resolveFirstProductActions,
  toFirstProductActionDestination,
  type FirstProductActionId,
} from "./services/firstLaunchProductActions";
...
import { settingsService, type AccessibilitySettings, type AppearanceSettings, type InterfaceScale, type ThemeMode, type ThemePreference } from "./services/settingsService";
import type { FirstLaunchOptionalStepId, FirstLaunchSetupState, FirstLaunchSetupStatePatch } from "./services/firstLaunchSetupState";
import { profileService } from "./services/profileService";
import { appearanceService } from "./services/appearanceService";
import { I18nProvider } from "./i18n";
import { normalizeUiLanguage } from "./services/localization/uiLanguages";
...
import { authErrorFallbackMessage, isAuthErrorCode } from "./services/auth/authErrorMap";
...
import { useVoiceParticipantsRailState } from "./state/useVoiceParticipantsRailState";
...
// Real Supabase rows use UUID ids. Placeholder entities (e.g. the "fallback-community"
// and "fallback-channel" shown before real data loads) are not queryable and would make
// PostgREST reject the request with a 400.
function isSupabaseEntityId(id: string): boolean {
  return UUID_ID_PATTERN.test(id);
}
...
const HavoocSupportHub = lazy(() => import("./features/havooc/HavoocSupportHub").then((module) => ({ default: module.HavoocSupportHub })));
...
type ActiveView = CommunityShellView | "mentionFeed" | "profile" | "directMessages" | "friends" | "savedMessages" | "discovery" | "events" | "live" | "support" | "havooc" | "rootPanel" | "publisherApply" | "publisherDashboard";

function toDesktopSafeLocation(view: ActiveView): DesktopSafeLocation | null {
  if (view === "mentionFeed") return "feed";
  if (view === "directMessages") return "messages";
  if (view === "community") return "communities";
  return null;
}

function fromDesktopSafeLocation(location: DesktopSafeLocation): ActiveView {
  if (location === "messages") return "directMessages";
  if (location === "communities") return "community";
  return "mentionFeed";
}
...
export function App() {
  ...
  const [firstLaunchSetup, setFirstLaunchSetup] = useState<FirstLaunchSetupState>(saved.firstLaunchSetup);
  ...
  const updateDesktopFirstLaunchSetup = (patch: FirstLaunchSetupStatePatch) => {
    const next = settingsService.updateFirstLaunchSetupState(patch);
    setFirstLaunchSetup(next.firstLaunchSetup);
    setAppearanceSettings(next.appearanceSettings);
    setTheme(appearanceService.resolveTheme(next.appearanceSettings.themeMode));
  };
  const updateDesktopFirstLaunchTheme = (themePreference: ThemePreference) => {
    const next = settingsService.updateAppearanceSettings({ themeMode: themePreference });
    setFirstLaunchSetup(next.firstLaunchSetup);
    setAppearanceSettings(next.appearanceSettings);
    setTheme(appearanceService.resolveTheme(next.appearanceSettings.themeMode));
  };
  const updateDesktopAccessibilitySettings = (partial: Partial<AccessibilitySettings>) => {
    const next = settingsService.updateAccessibilitySettings(partial);
    setAccessibilitySettings(next.accessibilitySettings);
  };
  const updateDesktopInterfaceScale = async (scale: InterfaceScale): Promise<boolean> => {
    const applied = await appearanceService.applyInterfaceScale(scale);
    if (!applied.ok) return false;
    updateDesktopAccessibilitySettings({ interfaceScale: applied.scale });
    return true;
  };
  const resetDesktopAppearanceStudio = async (): Promise<boolean> => {
    const defaults = settingsService.getAppearanceStudioDefaults();
    if (window.picomDesktop?.appearance?.setInterfaceScale) {
      const applied = await appearanceService.applyInterfaceScale(defaults.accessibility.interfaceScale);
      if (!applied.ok) return false;
    }
    const next = settingsService.resetAppearanceStudio();
    setFirstLaunchSetup(next.firstLaunchSetup);
    setAppearanceSettings(next.appearanceSettings);
    setAccessibilitySettings(next.accessibilitySettings);
    setTheme(appearanceService.resolveTheme(next.appearanceSettings.themeMode));
  ...
  const skipDesktopFirstLaunchStep = (stepId: FirstLaunchOptionalStepId) => {
    const next = settingsService.skipFirstLaunchSetupStep(stepId);
    setFirstLaunchSetup(next.firstLaunchSetup);
  };
  ...
  const desktopStartupRouteAppliedForUserRef = useRef<string | null>(null);
  const hasExplicitDesktopRouteIntentRef = useRef(false);
  ...
  const [communityJoinModalOpen, setCommunityJoinModalOpen] = useState(false);
  const [membershipSyncVersion, setMembershipSyncVersion] = useState(0);
  ...
  const isSupabaseMode = dataSourceService.getStatus().isSupabase;
  const [supabaseCommunityLoadState, setSupabaseCommunityLoadState] = useState<{
    userId: string | null;
    status: "idle" | "loading" | "ready" | "error";
  }>(() => ({ userId: null, status: isSupabaseMode ? "idle" : "ready" }));
  const {
    communities,
    appendLocalMessage,
    upsertLocalMessage,
  ...
  } = useLocalMessageState(isSupabaseMode ? [] : mockCommunities);
  const {
    activeCommunityId,
    activeCommunity,
    channels,
  ...
  const [guestAuthView, setGuestAuthView] = useState<"login" | "register" | "recovery">("login");
  ...
  useEffect(() => {
    if (authSession) setGuestAuthView("login");
  }, [authSession]);
  useEffect(() => {
    if (!authSession) {
  ...
  useEffect(() => {
    const expectedAccountId = authSession?.user?.id;
    let cancelled = false;
    // Account privacy is authoritative. A shared device cache must never make
    // a newly signed-in account visible before its server policy has loaded.
    globalPresenceService.setSharingEnabled(false);
    if (!expectedAccountId) return () => { cancelled = true; };
    void accountPrivacySetupService.hydrate().then((result) => {
      if (cancelled || result.status !== "ready" || result.snapshot.accountId !== expectedAccountId) return;
  ...
  useEffect(() => {
    ...
    if (!authSession?.user?.id) clearFirstLaunchHandoffIntent();
  }, [authSession?.user?.id]);

  useEffect(() => {
    if (!authReady || !authSession?.user || !firstLaunchSetup.completed || legalAcceptancePhase !== "accepted" || onboardingPhase !== "complete") {
      return;
    }
    const userId = authSession.user.id;
    if (desktopStartupRouteAppliedForUserRef.current === userId) return;
    let cancelled = false;
    void desktopBehaviorService.refresh().then((preferences) => {
      if (cancelled) return;
      if (hasExplicitDesktopRouteIntentRef.current) {
        clearFirstLaunchHandoffIntent();
      } else {
        const handoff = consumeFirstLaunchHandoffIntent(userId);
        if (handoff) {
          const action = resolveFirstProductActions({
          ...
          if (action) {
            ...
            if (destination.friendsTab) setFriendsViewTab(destination.friendsTab);
            if (destination.openCreateCommunity) setCreateCommunityOpen(true);
            desktopStartupRouteAppliedForUserRef.current = userId;
            return;
          }
        }
      }
      const target = resolveDesktopStartupDestination({
        firstLaunchRequired: !firstLaunchSetup.completed,
        authenticationRequired: !authSession,
        accountOnboardingRequired: onboardingPhase !== "complete",
        // A native deep link or notification marks this before passive startup
        // preferences are considered, so its route cannot be overwritten.
        explicitDestination: hasExplicitDesktopRouteIntentRef.current ? toDesktopSafeLocation(activeView) : null,
        startupDestination: preferences.startupDestination,
        lastSafeLocation: preferences.lastSafeLocation,
      });
      if (target) {
        setActiveView(fromDesktopSafeLocation(target));
        void desktopBehaviorService.rememberSafeLocation(target);
      }
      desktopStartupRouteAppliedForUserRef.current = userId;
    });
    return () => { cancelled = true; };
  }, [activeView, authReady, authSession, firstLaunchSetup.completed, legalAcceptancePhase, onboardingPhase]);

  useEffect(() => {
    const destination = toDesktopSafeLocation(activeView);
    if (!destination || !authSession?.user || !firstLaunchSetup.completed || onboardingPhase !== "complete" || legalAcceptancePhase !== "accepted") return;
    // Do not overwrite a persisted "last location" with the initial default
    // view before the passive startup preference has had a chance to restore it.
    if (!canPersistDesktopSafeLocation({
      userId: authSession.user.id,
      startupRouteAppliedForUserId: desktopStartupRouteAppliedForUserRef.current,
      destination,
    })) return;
    void desktopBehaviorService.rememberSafeLocation(destination);
  }, [activeView, authSession?.user?.id, firstLaunchSetup.completed, legalAcceptancePhase, onboardingPhase]);

  useEffect(() => {
    const openHavooc = () => setActiveView("havooc");
    const params = typeof window !== "undefined" ? new URLSearchParams(window.location.search) : null;
    if (params?.get("view") === "havooc" || (typeof window !== "undefined" && window.location.hash === "#havooc")) {
      openHavooc();
    }
    window.addEventListener("picom:open-havooc", openHavooc);
    (window as Window & { __picomOpenHavooc?: () => void }).__picomOpenHavooc = openHavooc;
    return () => {
      window.removeEventListener("picom:open-havooc", openHavooc);
      delete (window as Window & { __picomOpenHavooc?: () => void }).__picomOpenHavooc;
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (fromPath && fromPath !== activeView && !webNavigation.parsed.isAuthRoute) {
      ...
      if (fromPath === "live" || fromPath === "events" || fromPath === "friends" || fromPath === "savedMessages" || fromPath === "discovery" || fromPath === "profile" || fromPath === "havooc") {
  ...
  const joinedActiveVoiceChannel = useMemo(() => {
  ...
  const supabaseWorkspaceReady = !isSupabaseMode || (
    supabaseCommunityLoadState.status === "ready"
    && supabaseCommunityLoadState.userId === authSession?.user?.id
  );
  const hasHydratedActiveCommunity = !isSupabaseMode || (
    supabaseWorkspaceReady
    && communities.some((community) => community.id === activeCommunity.id)
  );
  const hasHydratedActiveChannel = !isSupabaseMode || (
    hasHydratedActiveCommunity
    && activeCommunity.categories.some((category) => category.channels.some((channel) => channel.id === activeChannel.id))
  );
  useEffect(() => {
  ...
  const realtimeStatus = useSupabaseMessageRealtime({
    enabled: Boolean(authSession) && !safeMode.active && activeView === "community" && activeCommunity.kind === "text" && hasHydratedActiveChannel,
    communityId: activeCommunity.id,
    channelId: activeChannel.id,
  ...
  useEffect(() => {
    if (activeCommunity.kind !== "text" || activeView !== "community" || !isActiveMessageListNearBottom || !hasHydratedActiveChannel) return;
  ...
  }, [activeCommunity.id, activeView, clearChannelUnread, displayedActiveChannel.id, hasHydratedActiveChannel, isActiveMessageListNearBottom, latestActiveMessageId]);

  useEffect(() => {
    if (activeCommunity.kind !== "text" || safeMode.active || !authSession || !isSupabaseMode || !hasHydratedActiveCommunity) return;
    let canceled = false;
    ...
    return () => { canceled = true; };
  }, [activeCommunity.id, authSession, hasHydratedActiveCommunity, isSupabaseMode, safeMode.active, setCommunityUnreadState]);
  ...
  useEffect(() => {
    ...
    communityService.listCommunities().then((result) => {
      ...
      if (!result.ok) {
        setSupabaseCommunityLoadState({ userId, status: "error" });
        pushToast(result.error.message, "error");
        return;
      }

      const nextCommunities = result.data.map((summary) => createCommunityFromSummary(summary, { includeTemplateChannels: false }));
      replaceCommunities(nextCommunities);
      setSupabaseCommunityLoadState({ userId, status: "ready" });
    });

    return () => {
      canceled = true;
    };
  }, [authSession?.user?.id, isSupabaseMode, pushToast, replaceCommunities]);
  ...
  }, [activeChannel.id, activeCommunity.id, activeCommunity.kind, authSession, hasHydratedActiveChannel, isSupabaseMode, mapMessageSummaryToMessage, pushToast, replaceChannelMessages, safeMode.active]);
  ...
  useEffect(() => {
    const handleDeepLinkAction = (action: DeepLinkAction) => {
      hasExplicitDesktopRouteIntentRef.current = true;
      ...
      if (action.type === "authOpen") {
        return;
      }
      if (action.type === "authCallback") {
        if (action.error) {
          const code = isAuthErrorCode(action.error) ? action.error : "AUTH_PROVIDER_FAILED";
          socialAuthService.rejectPendingCallback(action.provider, action.state, code);
          pushToast(authErrorFallbackMessage(code), code === "AUTH_CANCELLED" ? "info" : "error");
          return;
        }
        if (action.exchange && (action.provider === "steam" || action.provider === "epic")) {
          void socialAuthService.completeCustomOAuthCallback(action.exchange, action.provider).then((result) => {
            if (result.ok) {
              pushToast(result.data.linked ? `${action.provider === "epic" ? "Epic Games" : "Steam"} connected.` : "Signed in to Picom.", "success");
              return;
            }
            pushToast(authErrorFallbackMessage(result.code), "error");
          });
          return;
        }
        if (action.code && (action.provider === "google" || action.provider === "apple")) {
          void socialAuthService.completeOAuthCallback(action.code, action.state, action.provider).then((result) => {
            pushToast(result.ok ? "Signed in to Picom." : authErrorFallbackMessage(result.code), result.ok ? "success" : "error");
          });
          return;
        }
        pushToast(authErrorFallbackMessage("AUTH_CALLBACK_FAILED"), "error");
        return;
      }
      ...
      if (action.type === "friends") {
        if (activeView !== "friends") {
          friendsReturnViewRef.current = activeView;
        }
  ...
  }, [activeView, closeTransientOverlays, directConversations, openPalette, openSystemStatusPage, pushToast]);
  ...
  const toggleMentionReaction = useCallback((id: string, emoji: string) => {
    ...
    const existingReaction = previousReactions.find((reaction) => reaction.emoji === emoji);
    const wasReacted = Boolean(existingReaction?.reactedByCurrentUser);

    const sortReactions = (reactions: typeof previousReactions) =>
      reactions
    ...
    const buildOptimisticReactions = () => {
      if (wasReacted) {
        if (!existingReaction) return previousReactions;
        if (existingReaction.count <= 1) return sortReactions(previousReactions.filter((reaction) => reaction.emoji !== emoji));
        return sortReactions(previousReactions.map((reaction) =>
      ...
      return sortReactions([...previousReactions, { emoji, count: 1, reactedByCurrentUser: true }]);
    };

    const optimisticReactions = buildOptimisticReactions();
    setMentionItems((current) => current.map((candidate) => candidate.id === id ? { ...candidate, reactions: optimisticReactions } : candidate));

    void (async () => {
      const result = wasReacted
        ? await reactionService.removeReaction({ messageId: item.messageId, emoji })
        : await reactionService.addReaction({ messageId: item.messageId, emoji });
      if (!result.ok) {
      ...
      setMentionItems((current) => current.map((candidate) => {
        if (candidate.id !== id) return candidate;
        const nextReactions = (candidate.reactions ?? []).filter((reaction) => reaction.emoji !== emoji);
        ...
        return { ...candidate, reactions: sortReactions(nextReactions) };
      }));
    })();
  ...
  const joinActiveVoiceRoom = useCallback(async () => {
    ...
    if (!authSession) {
      pushToast("Sign in before entering voice.", "error");
      return;
    }

    if (!communityAccess.isMember) {
      setCommunityJoinModalOpen(true);
      return;
    }

    let channelId = displayedActiveChannel.id;
    let channelName = displayedActiveChannel.name;
    if (dataSourceService.getStatus().isSupabase) {
      const remoteChannels = await channelService.listChannels(activeCommunity.id);
      if (!remoteChannels.ok) {
        pushToast(remoteChannels.error.message, "error");
        return;
      }
      const normalizedName = displayedActiveChannel.name.trim().toLowerCase();
      const remoteChannel = remoteChannels.data.find((channel) => channel.type === "voice" && channel.id === displayedActiveChannel.id)
        ?? remoteChannels.data.find((channel) => channel.type === "voice" && channel.name.trim().toLowerCase() === normalizedName);
      if (!remoteChannel) {
        pushToast("This voice channel has not synchronized with Supabase yet.", "error");
        return;
      }
  ...
  const toggleActiveVoiceCamera = useCallback(() => {
    void import("./services/voiceService").then(({ voiceService }) =>
      voiceService.setCameraEnabled(!voiceSnapshot.cameraEnabled).then((result) => {
        if (!result.ok) pushToast(result.error.message, "error");
      }),
    );
  }, [pushToast, voiceSnapshot.cameraEnabled]);
  ...
  const openConnectedVoiceRoom = useCallback(() => {
    ...
    const targetCommunity = communities.find((candidate) => candidate.id === roomContext.communityId);
    const targetChannel = targetCommunity?.categories
  ...
  const closeFriendsView = useCallback(() => {
    setActiveView(friendsReturnViewRef.current);
    closeTransientOverlays();
  }, [closeTransientOverlays]);
  ...
  const finishDesktopFirstLaunchSetup = useCallback(async (options?: Readonly<{ action?: FirstProductActionId }>) => {
    const result = settingsService.completeFirstLaunchSetup();
    if (!result.ok) return { ok: false };
    releaseFirstLaunchMediaResources();
    setFirstLaunchSetup(result.settings.firstLaunchSetup);
    setAppearanceSettings(result.settings.appearanceSettings);
    setTheme(appearanceService.resolveTheme(result.settings.appearanceSettings.themeMode));
    const eligible = Boolean(authSession?.user)
      && legalAcceptancePhase === "accepted"
      && onboardingPhase === "complete";
    if (eligible && options?.action) {
      setFirstLaunchHandoffIntent(options.action, authSession?.user?.id ?? null);
    }
    return { ok: true };
  }, [authSession?.user, legalAcceptancePhase, onboardingPhase]);

  if (!safeMode.active && !firstLaunchSetup.completed) {
    return (
      <I18nProvider locale={normalizeUiLanguage(appearanceSettings.language)}>
        <DesktopAppShell>
          <WindowTitleBar theme={theme} onToggleTheme={() => updateDesktopFirstLaunchTheme(theme === "light" ? "dark" : "light")} onOpenSearch={() => undefined} />
          <FirstLaunchSetup state={firstLaunchSetup} appearanceSettings={appearanceSettings} accessibilitySettings={accessibilitySettings} session={{ authenticated: Boolean(authSession), accountId: authSession?.user?.id ?? null, legalAccepted: legalAcceptancePhase === "accepted", onboardingComplete: onboardingPhase === "complete" }} onStateChange={updateDesktopFirstLaunchSetup} onThemeChange={updateDesktopFirstLaunchTheme} onAppearanceChange={(partial) => { const next = settingsService.updateAppearanceSettings(partial); setFirstLaunchSetup(next.firstLaunchSetup); setAppearanceSettings(next.appearanceSettings); setTheme(appearanceService.resolveTheme(next.appearanceSettings.themeMode)); }} onAccessibilityChange={updateDesktopAccessibilitySettings} onInterfaceScaleChange={updateDesktopInterfaceScale} onResetAppearance={resetDesktopAppearanceStudio} onSkip={skipDesktopFirstLaunchStep} onComplete={finishDesktopFirstLaunchSetup} />
        </DesktopAppShell>
      </I18nProvider>
    );
  }
  ...
  if (!authReady) {
    return (
      <DesktopAppShell>
        ...
        <main className="first-run-onboarding onboarding-loading">
          <span className="onboarding-welcome-orb"><AppIcon name="user" size="xl" /></span>
          <strong>Restoring your Picom session...</strong>
        </main>
      </DesktopAppShell>
    );
  }
        ...
        <DesktopAppShell>
          ...
          {maintenanceStatus.status === "maintenance" ? (
            ...
            <RegisterScreen
              ...
              errorCode={authErrorCode}
              notice={authNotice}
              onSubmit={async (email, password, displayName, acceptedLegalVersion) => {
                await handleRegister(email, password, displayName, acceptedLegalVersion);
              }}
              onSwitchToLogin={() => { clearAuthError(); setGuestAuthView("login"); }}
            />
          ) : guestAuthView === "recovery" ? (
            <ForgotPasswordScreen theme={theme} onBack={() => { clearAuthError(); setGuestAuthView("login"); }} />
          ) : (
            <LoginScreen
              theme={theme}
              loading={!authReady || authLoading}
              ...
              onCreateAccount={() => { clearAuthError(); setGuestAuthView("register"); }}
              onForgotPassword={() => { clearAuthError(); setGuestAuthView("recovery"); }}
  ...
  if (isSupabaseMode && supabaseCommunityLoadState.status !== "ready") {
    return (
      <DesktopAppShell>
        <WindowTitleBar theme={theme} onToggleTheme={toggleTheme} onOpenSearch={() => undefined} />
        <main className="first-run-onboarding onboarding-loading">
          <span className="onboarding-welcome-orb"><AppIcon name="home" size="xl" /></span>
          <strong>{supabaseCommunityLoadState.status === "error" ? "Picom could not load your communities." : "Loading your Picom workspace..."}</strong>
        </main>
        <ToastStack toasts={toasts} onDismiss={dismissToast} />
      </DesktopAppShell>
    );
  ...
  const handleJoinCommunity = async (rulesAcceptance: { rulesVersion: string; acceptedAt: string } | null) => {
    ...
    const joinedMember = {
      ...result.data.member,
      roleIds: result.data.member.roleIds?.length
        ? [...result.data.member.roleIds]
        : result.data.member.roleId
          ? [result.data.member.roleId]
          : undefined,
    };
  ...
  const handleInviteAccepted = async (communityId: string, member: Member, status: InviteAcceptanceStatus, preview: CommunityInvitePreview) => {
    ...
    if (!target) {
      ...
      target = addCommunity(createCommunityFromSummary(summary, { includeTemplateChannels: dataSourceService.getStatus().isMock }));
    }
  ...
  const handleCreateCommunity = async (value: CreateCommunityFormValue): Promise<CreateCommunitySubmitResult> => {
    ...
    if (value.iconFile) {
      const validation = communityBrandingService.validate(value.iconFile, "icon");
      if (!validation.ok) {
        return { ok: false, error: validation.message };
      }

      if (dataSourceService.getStatus().isMock) {
        const uploaded = await communityBrandingService.upload("draft", "icon", value.iconFile);
        if (!uploaded.ok) {
    ...
    const { iconFile, ...createInput } = value;
    const result = await communityService.createCommunity({ ...createInput, iconUrl });
    ...
    if (value.iconFile && dataSourceService.getStatus().isSupabase) {
      const uploaded = await communityBrandingService.upload(summary.id, "icon", value.iconFile);
      if (uploaded.ok) {
        const updated = await communityService.updateCommunitySettings({ id: summary.id, iconUrl: uploaded.data.url });
        if (updated.ok) {
    ...
    <I18nProvider locale={normalizeUiLanguage(appearanceSettings.language)}>
        ...
        <div className="desktop-frame">
          ...
          {activeView === "rootPanel" ? (
            ...
            <DeferredViewBoundary label="Opening Help and Support">
              <HelpSupportWorkspace pushToast={pushToast} />
            </DeferredViewBoundary>
          ) : activeView === "havooc" ? (
            <DeferredViewBoundary label="Opening HAVOOC Support Hub">
              <HavoocSupportHub
                authenticated={Boolean(authSession?.user)}
                onBack={() => setActiveView("mentionFeed")}
                onRequestAuth={() => {
                  pushToast("Sign in to leave a support note.", "info");
                  setActiveView("mentionFeed");
                }}
            ...
            <CommunityWorkspace serverRail={communityServerRail}>
              <CommunitySidebar
                ...
                voiceState={voiceSnapshot}
                onToggleVoiceMute={toggleActiveVoiceMute}
                onToggleVoiceDeafen={toggleActiveVoiceDeafen}
                onToggleVoiceCamera={toggleActiveVoiceCamera}
              ...
              {displayedActiveChannel.type === "voice" ? (
                ...
                <VoiceRoomView
                  ...
                  canSpeak={Boolean(voiceSnapshot.canSpeak) && communityAccess.isMember}
              ...
              <ChatMain
                ...
                onOpenSavedMessages={() => setActiveView("savedMessages")}
                onOpenMentionFeed={() => setActiveView("mentionFeed")}
                ...
                onOpenJoinCommunity={() => setCommunityJoinModalOpen(true)}
                pushToast={pushToast}
              />
              )}
      ...
      {communityJoinModalOpen ? (
        <CommunityJoinModal
          ...
          currentUserId={currentUser.userId}
          isAuthenticated={Boolean(authSession)}
          onClose={() => setCommunityJoinModalOpen(false)}
          onConfirm={async (acceptance) => {
            const joined = await handleJoinCommunity(acceptance);
            if (joined) setCommunityJoinModalOpen(false);
            return joined;
          }}
        />
      ) : null}
